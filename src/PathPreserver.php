    ...
    public function preserve() 
    {
        foreach ($this->installPaths as $installPath) {
            ...
            foreach ($this->preservePaths as $path) {
                ...
                if ($this->fileExists($path) && strpos($normalizedPath, $installPathNormalized) === 0) {
    ...
    public function rollback() 
    {
        ...
        foreach ($this->backups as $original => $backup_location) {
            ...
            if ($this->fileExists($original)) {
                ...
                $this->io->write(
                    sprintf(
                        '<comment>Files of installed package were overwritten with preserved path %s!</comment>',
                        $original
                    ),
                    true
                );
